...
class LibraryApp:
    ...
    def create_widgets(self):
        ...
        self.v = IntVar()
        Radiobutton(self.root, text='with status', variable=self.v, value=1,
                    command=lambda: self.not_show_column(True)).grid(row=2, column=2)
        Radiobutton(self.root, text='no status', variable=self.v, value=2,
                    command=lambda: self.not_show_column(False)).grid(row=3, column=2)
    ...
    def image_drawer(self):
        ...
        self.rect = None
        self.rect_id = None
        self.label = Label(new_window, text="Dimensions: Width x Height", font=("Helvetica", 12))
        self.label.grid()
        self.text_label = Label(new_window, text="Recognized Text: ", font=("Helvetica", 12))
    ...
    def load_image(self):
        ...
        if file_path:
            if self.library_manager.open_library(file_path):
                ...
                messagebox.showinfo("Error", "Unable to load image.")
